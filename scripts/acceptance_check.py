...
def request_text(url: str, timeout: int = 30) -> tuple[int, str, dict[str, str]]:
    request = urllib.request.Request(url, method="GET")
    with urllib.request.urlopen(request, timeout=timeout) as response:
        return response.status, response.read().decode("utf-8", errors="replace"), dict(response.headers.items())


def head_status(url: str, timeout: int = 30) -> tuple[int, str]:
    last_error: Exception | None = None
...
def main() -> int:
    ...
    parser.add_argument("--require-real-platforms", action="store_true", help="Also require real WeChat/WeCom/Douyin/Taobao/PDD/Xianyu windows to be open and readable.")
    ...
    try:
        snapshot = get_json(f"{args.base_url.rstrip('/')}/api/ecommerce/snapshot")
        checks["ecommerce_snapshot_api"] = {
            "ok": (
                isinstance(snapshot, dict)
                and isinstance(snapshot.get("platforms"), list)
                and isinstance(snapshot.get("products"), list)
                and isinstance(snapshot.get("automation_jobs"), list)
            ),
            "platforms": len(snapshot.get("platforms", [])) if isinstance(snapshot, dict) else None,
            "products": len(snapshot.get("products", [])) if isinstance(snapshot, dict) else None,
        }
    except Exception as exc:
        checks["ecommerce_snapshot_api"] = {"ok": False, "error": str(exc)}

    try:
        widget_status, widget_text, widget_headers = request_text(f"{args.base_url.rstrip('/')}/api/widget.js?merchant_code=WJAIKF001")
        checks["widget_script"] = {
            "ok": widget_status == 200 and "fetch" in widget_text and "/widget/session" in widget_text and "/widget/message" in widget_text,
            "status": widget_status,
            "content_type": widget_headers.get("Content-Type", ""),
            "length": len(widget_text),
        }
    except Exception as exc:
        checks["widget_script"] = {"ok": False, "error": str(exc)}

    try:
        product_pack = post_json(
            f"{args.base_url.rstrip('/')}/api/product-media/pack",
            {
                "product_name": "验收保湿精华",
                "category": "美妆护肤",
                "platform": "douyin",
                "price": "129.00",
                "selling_points": "补水保湿,清爽不粘,敏感肌可用",
                "audience": "18-35岁护肤用户",
                "visual_style": "爆款素材批量生成风",
                "call_to_action": "保存上架资料",
                "render_video": False,
            },
            timeout=120,
        )
        main_url = f"{args.base_url.rstrip('/')}{product_pack.get('main_image_url', '')}"
        detail_url = f"{args.base_url.rstrip('/')}{product_pack.get('detail_image_url', '')}"
        main_status, main_type = head_status(main_url)
        detail_status, detail_type = head_status(detail_url)
        checks["product_media_pack"] = {
            "ok": (
                product_pack.get("product_name") == "验收保湿精华"
                and str(product_pack.get("main_image_url", "")).endswith(".png")
                and str(product_pack.get("detail_image_url", "")).endswith(".png")
                and main_status == 200
                and detail_status == 200
                and "image/png" in main_type
                and "image/png" in detail_type
                and (product_pack.get("listing_draft") or {}).get("publish_boundary") == "save_draft_only"
            ),
            "main": {"url": main_url, "status": main_status, "type": main_type},
            "detail": {"url": detail_url, "status": detail_status, "type": detail_type},
            "video_status": product_pack.get("video_status"),
        }
    except Exception as exc:
        checks["product_media_pack"] = {"ok": False, "error": str(exc)}

    try:
        source = (root / "src" / "main.tsx").read_text(encoding="utf-8", errors="replace")
        forbidden_seller_copy = [
            "适合你卖",
            "客户和报价",
            "可卖套餐",
            "成交话术",
        ...
        found = [text for text in forbidden_seller_copy if text in source]
        checks["seller_copy_clean"] = {"ok": not found, "found": found}
    except Exception as exc:
        checks["seller_copy_clean"] = {"ok": False, "error": str(exc)}

    channels: dict[str, Any] = {}
    for channel in ["wechat", "wechat_work", "douyin_dm", "taobao", "pdd", "xianyu"]:
        try:
            data = post_json(
