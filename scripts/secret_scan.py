...
CODE_VALUE_ALLOWLIST = {"get_settings", "authorization", "payload", "request", "response"}
PATTERNS = [
        ...
        for pattern in PATTERNS:
            for match in pattern.finditer(text):
                ...
                if any(token in matched for token in PLACEHOLDERS):
                    continue
                value = match.group(0).split("=", 1)[-1].split(":", 1)[-1].strip().strip("'\"").lower()
                if value in CODE_VALUE_ALLOWLIST:
                    continue
