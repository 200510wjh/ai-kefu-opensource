...
- 会话收件箱：查看真实会话和 AI 回复。
- 桌面自动客服：微信/抖音/千牛/拼多多/闲鱼桌面助手说明和命令。
- 网页气泡：生成网站接入代码。
...
它会自动查找微信、抖音、千牛、拼多多、闲鱼客户端或快捷方式，能启动就尝试启动，并生成中文报告：
...
1. 先打开微信、抖音、千牛、拼多多或闲鱼客服窗口。
2. 回到“AI 自动客服监听器”，点“刷新窗口”。
3. 在列表里选中真实客服聊天窗口。
4. 平台会自动识别；没识别时手动选微信/抖音/淘宝/拼多多/闲鱼。
...
7. 如果目标客服窗口无法切到前台，系统会只复制回复到剪贴板，不会粘贴或发送到错误窗口。
...
它不替代真实平台窗口验收，只证明“示例聊天 + 本地知识库 + 五个平台渠道 + AI 回复 + 禁用承诺检查”这条链路是通的。
...
这不是代码坏了，而是说明当前机器没有可验收的真实平台窗口。打开微信、抖音、千牛、拼多多、闲鱼客服聊天窗口后再跑。
...
- 没有官方 API 权限时，微信/抖音/淘宝/拼多多/闲鱼不能保证后台无人值守读取所有私信。
