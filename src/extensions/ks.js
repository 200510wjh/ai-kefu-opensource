...
(function() {
  ...
  function log(...args) {
    if (CONFIG.debug) {
      console.log(`[AI客服-快手]`, new Date().toLocaleTimeString('zh-CN'), ...args);
    }
  }
  ...
  function insertAIResponse(replyText) {
    try {
      ...
      if (inputElement) {
        ...
        setTimeout(() => {
          const sendBtn = document.querySelector('[class*="send"], [class*="submit"] button, .send-btn');
          if (sendBtn) {
            sendBtn.click();
            log('已发送');
          }
