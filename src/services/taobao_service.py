...
class TaobaoService:
    ...
    async def _fetch_messages(self) -> list:
        ...
        try:
            # 访问消息页面
            await self._page.goto("https://myseller.taobao.com/portal.htm", wait_until="networkidle")
    ...
    async def send_reply(self, customer_id: str, reply: str) -> bool:
        ...
        try:
            ...
            logger.info(f"淘宝消息发送成功: {customer_id}")
            return True
        except Exception as e:
            logger.error(f"淘宝消息发送失败: {e}")
            return False
